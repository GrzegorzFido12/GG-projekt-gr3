...
class P3(Production):
    ...
    This production finds a shared edge (B=0, meaning it's internal/not boundary) 
    that is marked for refinement (R=1) and splits it by:
    ...
    4. Both new edges remain shared (B=1, meaning they belong to boundary calculation)
    
    Left side: An edge E with B=1 (shared) and R=1 (marked for refinement)
    Right side: Two edges E with B=1 and R=0, connected through a new hanging node
    """
    
    def get_left_side(self) -> Graph:
        """
        Returns a graph representing the left side of production P3.
        The left side is a shared edge (B1=0) marked for refinement (R=1).
        """
        g = Graph()
        n1 = Node(0, 0,"v1")
        n2 = Node(1, 0,"v2")
        g.add_node(n1)
        g.add_node(n2)
        # B1=0 means this is a shared edge (not on the outer boundary)
    ...
    def get_right_side(self, matched: Graph, level: int) -> Graph:
        ...
        # Find the matched edge that is a shared edge marked for refinement
        matched_edge = None
        ...
        # Create two new edges, both shared (B=1) but not marked for refinement (R=0)
    ...
    def can_apply(self, graph: Graph) -> bool:
        ...
        Returns True if there exists a shared edge (B=0) marked for refinement (R=1).
        """
    ...
    def find_match(self, graph: Graph) -> Optional[HyperEdge]:
        ...
        Returns the first shared edge (B=0) marked for refinement (R=1).
        """
        for edge in graph.hyperedges:
            if edge.hypertag == "E" and edge.R == 1 and edge.B == 0:
                return edge
    ...
    def is_isomorphic_to_left_side(self, subgraph: Graph) -> bool:
        ...
        - Exactly 2 nodes
        - Exactly 1 edge E with B=0 and R=1
        """
        ...
        # Edge must be labeled 'E', be a shared edge (B=0), and marked for refinement (R=1)
        if edge.hypertag != "E":
            return False
        if edge.B != 0:
            return False
