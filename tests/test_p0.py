...
def test_pentagon():
    ...
    """
    g = Graph()
    nodes = [Node(0, 0, "x1"), Node(1, 0, "x2"), Node(2, 1, "x3"), Node(1, 2, "x4"), Node(0, 1, "x5")]
    for i in range(5):
        next_i = (i + 1) % 5
        g.add_edge(HyperEdge((nodes[i], nodes[next_i]), "E", R=0, B=1))
...
def test_two_adjacent_squares_mixed_state():
    ...
    left_q = [e for e in g.hyperedges if e.hypertag == "Q" and set(e.nodes) == set(left_nodes)][0]
    right_q = [e for e in g.hyperedges if e.hypertag == "Q" and set(e.nodes) == set(right_nodes)][0]
...
def test_missing_boundary_edge():
    ...
    g = Graph()
    nodes = [
        Node(0, 0, "n1"), Node(1, 0, "n2"), 
        Node(1, 1, "n3"), Node(0, 1, "n4")
    ]
...
def test_double_edge_compensating_missing_edge():
    ...
    assert result == 0, "Production applied to an open loop because duplicate edges fooled the counter!"
...
if __name__ == "__main__":
    ...
    tests = [
        ...
        ("DOUBLE EDGE COMPENSATING MISSING EDGE TEST", test_double_edge_compensating_missing_edge),
