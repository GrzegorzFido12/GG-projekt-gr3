...
def create_square_with_boundary_edges():
    ...
    g = Graph()
    nodes = [Node(0, 0, "n1"), Node(4, 0, "n2"), Node(4, 4, "n3"), Node(0, 4, "n4")]
    for n in nodes:
        g.add_node(n)
...
def create_complex_mesh():
    ...
    # Boundary edges
    g.add_edge(
        HyperEdge((nodes[0], nodes[1]), "E", boundary=True, R=1, B=1)
    )  # bottom-left, marked
    g.add_edge(
        HyperEdge((nodes[1], nodes[2]), "E", boundary=True, R=0, B=1)
    )  # bottom-right
    ...
    g.add_edge(
        HyperEdge((nodes[3], nodes[4]), "E", boundary=True, R=0, B=1)
    )  # top-left
    g.add_edge(
        HyperEdge((nodes[4], nodes[5]), "E", boundary=True, R=0, B=1)
    )  # top-right
...
def test_p4_on_complex_mesh():
    ...
    marked_boundary_edges = [
        e for e in g.hyperedges if e.hypertag == "E" and e.R == 1 and e.B == 1
    ]
...
def test_p4_preserves_other_edges():
    ...
    [
        ...
        if e.hypertag == "E"
        and e.R == 0
        and "h_" not in e.nodes[0].label
        and "h_" not in e.nodes[1].label
    ]
...
def test_p4_hanging_node_position():
    ...
    assert hanging_nodes[0].x == expected_x, (
        f"Expected x={expected_x}, got {hanging_nodes[0].x}"
    )
    assert hanging_nodes[0].y == expected_y, (
        f"Expected y={expected_y}, got {hanging_nodes[0].y}"
    )
...
def test_p4_find_all_matches():
    ...
    g = Graph()
    nodes = [Node(0, 0, "a"), Node(4, 0, "b"), Node(8, 0, "c"), Node(12, 0, "d")]
    for n in nodes:
        g.add_node(n)
...
def test_p4_triangle_boundary():
    ...
    g = Graph()
    nodes = [Node(0, 0, "t1"), Node(6, 0, "t2"), Node(3, 5, "t3")]
    for n in nodes:
        g.add_node(n)
...
def test_p4_visualization_correctness():
    ...
    assert os.path.exists(output_path), (
        f"Visualization file {output_path} was not created"
    )
...
def test_p4_complete_right_side_verification():
    ...
    for edge in edges:
        ...
        assert len(edge.nodes) == 2, (
            f"Edge should connect 2 nodes, got {len(edge.nodes)}"
        )
...
if __name__ == "__main__":
    tests = [
        ...
        (
            "P4 REMOVE RANDOM EDGE FROM VALID GRAPH",
            test_p4_remove_random_edge_from_valid_graph,
        ),
        ("P4 VISUALIZATION CORRECTNESS", test_p4_visualization_correctness),
        (
            "P4 COMPLETE RIGHT SIDE VERIFICATION",
            test_p4_complete_right_side_verification,
        ),
    ]
